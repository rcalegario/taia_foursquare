...
# freq = new_social %>% group_by(user_id) %>% summarise(freq = n())
# fdp = freq %>% filter(freq > 1500) %>% select(user_id)
# new_social2 = new_social %>% anti_join(fdp, by="user_id") 
# colnames(fdp) = c("friend_id")
# new_social2 = new_social2 %>% anti_join(fdp, by="friend_id")
# write.table(new_social, file="social_fdp.csv", sep = ";", quote = FALSE, row.names = FALSE)
